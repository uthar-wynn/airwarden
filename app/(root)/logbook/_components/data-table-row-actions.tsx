...
import { logbookSchema } from "../_data/schema"
...
export function DataTableRowActions<TData>({
    ...
    const logbook = logbookSchema.parse(row.original)
    const router = useRouter()

    const onAction = async (e: React.MouseEvent) => {
        e.stopPropagation()
        router.push(`/logbook/${logbook.id}/edit`)
    }

    const onRemove = async () => {
        try { // TODO: correct DELETE URL
            await axios.delete(`/api/locations/${logbook.id}`)
                .then(() => {
